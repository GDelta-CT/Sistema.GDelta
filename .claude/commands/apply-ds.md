...
Padrão: se nenhum `name` for dado, use `default` (a marca Graphite).
...
5. Para usar num build: passe `--tokens design-systems/<name>/design-tokens.json` ao `/build-experience` ou `/build-product`. Sem `--tokens`, os builds já usam `design-systems/default/` (Graphite).
